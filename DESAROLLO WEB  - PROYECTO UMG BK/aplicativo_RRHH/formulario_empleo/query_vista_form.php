...
FROM solicitud_empleo WHERE estatus = 'Pendiente';
");
$query->execute();
...
if (isset($_POST['registro_id']) && isset($_POST['accion_boton'])) {
    $registro_id = $_POST['registro_id'];
    $accion_boton = $_POST['accion_boton'];

    try {
        if ($accion_boton === 'rechazarSolicitud') {
            $updateQuery = $conexionbd->prepare("UPDATE solicitud_empleo SET estatus = 'Rechazada' WHERE id = :id");
            $updateQuery->bindParam(':id', $registro_id);
            $updateQuery->execute();
            $_SESSION['alert'] = ['type' => 'success', 'message' => 'La solicitud ha sido rechazada.'];
        } elseif ($accion_boton === 'iniciar_proceso_contratacion') {
            $updateQuery = $conexionbd->prepare("UPDATE solicitud_empleo SET estatus = 'En Proceso' WHERE id = :id");
            $updateQuery->bindParam(':id', $registro_id);
            $updateQuery->execute();
            $_SESSION['alert'] = ['type' => 'success', 'message' => 'El proceso de contratación ha sido iniciado.'];
        }

        // Redirigir después de la acción
        header("Location: form_completo.php");
        exit();
    } catch (Exception $e) {
